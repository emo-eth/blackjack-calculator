...
import calculator.classic.ClassicGameState.actionDouble
import calculator.classic.ClassicGameState.actionHit
import calculator.classic.ClassicGameState.actionInsurance
import calculator.classic.ClassicGameState.actionSplit
import calculator.classic.ClassicGameState.actionStand
import calculator.classic.ClassicGameState.dealer
import calculator.classic.ClassicGameState.insurance
import calculator.classic.ClassicGameState.player
import calculator.classic.ClassicGameState.split
import calculator.classic.ClassicGameState.splitAces
...
object ClassicGameStateModel : AbstractGameStateModel() {
    ...
    override fun initialize() {
        transaction {
            ...
            SchemaUtils.create(ClassicGameState)
        }
    ...
    override fun getHand(
        ...
        val resultRow = transaction {
            ClassicGameState.select {
                (ClassicGameState.insurance.eq(insurance) and
                        ClassicGameState.split.eq(if (split == null) "" else split.toUTF8().toString(Charset.defaultCharset())) and
                        ClassicGameState.dealer.eq(dealer.toUTF8()[0].toChar()) and
                        ClassicGameState.splitAces.eq(splitAces) and
                        ClassicGameState.player.eq(player.toUTF8().toString(Charset.defaultCharset())))
            }.firstOrNull()
        } ?: return null
    ...
    override fun insertHand(
        ...
        try {
            transaction {
                ClassicGameState.batchInsert(batchInsertMap.entries, true) { entry ->
                    ...
                    this[ClassicGameState.insurance] = insuranceRow
                    this[ClassicGameState.split] = splitString ?: ""
                    this[ClassicGameState.player] = playerString
                    this[ClassicGameState.dealer] = dealerString[0]
                    this[ClassicGameState.splitAces] = splitAcesRow
